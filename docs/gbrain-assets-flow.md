# G-Brain 资产灌入管线（Design v1）

> 状态：设计稿 · 作者：ruby · 日期：2026-08-10
> 原则：**先宣告，后灌入；先定义，后执行；引文必须可验证**

## 0. 背景与问题

高权重文档（论文 PDF、典籍全文、案卷）应落在 G-Brain 单源。当前基础设施三个病：

1. **没宣告**——agent 不知道上哪找什么（本次找不到鄂霍次克海/牛津剑桥/石器时代 PDF 就是症状）
...
- 本文档：宣告层的一部分，落 ai-ops docs/ 供所有 agent 引用

## 8. 待办（实施清单——由用户确认优先级后执行）
