...
ROBERTS_STEPS = [
    ...
    ("prosecutor_asst1", "受公诉人指派主动进攻质证：抓住辩方尚未回答的三个命门逐一逼问——①1000万为何借道第三方、拆成10笔×100万、分10次归还？单笔大额会触发监管监测，拆单恰是规避监测的结构性动作，若问心无愧为何不堂堂正正走公开程序？②尊长作为党组成员，是否向组织报备过此次筹资？不报备本身就是对‘职务影响可能外溢’的明知！③水单只是归还记录，谁证明筹款来源清白？要求辩方当庭出示筹款来源凭证！同时调用内部研判《反方弹药·恶意揣测》中的立体武器逐一逼问：无息不是善良而是定价——2016年山河四省资金荒，民间拆借司法保护上限年化36%，1000万一年利息360万，放弃利息等于把法律保护范围内实打实的对价藏进人情债；为何选春节启动——监督空窗与情理绑架的双重踩点；为何3个月内全部还清——他预判过庭审，还款节奏是诉讼预案而非救急节奏；中间人是谁的防火墙——让资金来源查无来路。语气沉静但刀刀见血，绝不自我软化！归还令牌！"),
...
class RobertTokenRingEngine:
    ...
    def _build_context_blocks(self, seat_key):
        ...
        for m in self.shared_context:
            if m.get("team") == "pretrial":
                ...
                break
        if seat_key in ("prosecutor_chief", "prosecutor_asst1", "topaz", "carbonado"):
            for m in self.shared_context:
                if m.get("team") == "prosecution_intel":
                    blocks.append(("反方内部研判《恶意揣测》情报（仅公诉席可见·只可转化为当庭逼问弹药·不得直接朗读）", m["content"]))
                    break
    ...
    def draft_official_indictment(self):
        ...
        prompt = (
            ...
            "1. 拆单规避监测：1000万拆为10笔×100万经中间人划转，而非单笔直转，系规避大额交易监测的典型拆单结构，证明资金运作的刻意隐蔽性；\n"
            "2. 中间人断点设计：多源头→单一中间人→目标企业的路由结构，使资金来源无法直接追溯至被告人的职务网络，系防火墙式断点设计；\n"
            "3. 无息即人情定价：2016年山河四省资金荒，民间拆借司法保护上限为年化36%（月息3分），1000万一年利息达360万元，被告人放弃全部利息，是以人情债替代金钱对价——主张按上限报，对方砍到24%仍是240万，无论怎么砍对价事实都成立；人情债无账目、不可审计；\n"
            "4. 春节制度盲区：2016年春节期间启动、年内完成还款，整个运作横跨组织监督最松弛的年度空窗，系日历级别的踩点；\n"
            "5. 程序缺口：全程不报备、不批准、不公开，作为党组成员明知廉洁从业规定仍刻意绕过组织监督，属于职务影响力外溢的典型路径；\n"
            "6. 破窗风险：若此例可开，私情为名、信用为桥、程序为障的行为将腐蚀国企监管体系。\n"
