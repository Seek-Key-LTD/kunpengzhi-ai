...
function hasConfigObject(value: unknown): value is Record<string, unknown> {
  return !!value && typeof value === "object" && !Array.isArray(value);
}

function getConfigEnvRecord(api: OpenClawPluginApi): Record<string, unknown> | undefined {
  const envCfg = (api.config as Record<string, unknown> | undefined)?.env;
  return hasConfigObject(envCfg) ? envCfg : undefined;
}

function getMem0EnvValue(api: OpenClawPluginApi, key: string): string | undefined {
  const processValue = process.env[key];
  if (typeof processValue === "string" && processValue) {
    return processValue;
  }

  const envCfg = getConfigEnvRecord(api);
  const varsCfg = hasConfigObject(envCfg?.vars) ? envCfg?.vars : undefined;
  const varsValue = varsCfg?.[key];
  if (typeof varsValue === "string" && varsValue) {
    return varsValue;
  }

  const directValue = envCfg?.[key];
  return typeof directValue === "string" && directValue ? directValue : undefined;
}
...
function parseOptionalJsonObject(value: string | undefined): Record<string, unknown> | undefined {
  ...
  try {
    ...
    return hasConfigObject(parsed) ? parsed : undefined;
  } catch {
    return undefined;
  }
}

function resolveMem0EnvConfig(api: OpenClawPluginApi): unknown {
  const mode = getMem0EnvValue(api, "MEM0_MODE");
  const apiKey = getMem0EnvValue(api, "MEM0_API_KEY");
  const userId = getMem0EnvValue(api, "MEM0_USER_ID");
  const host = getMem0EnvValue(api, "MEM0_HOST");
  const agentId = getMem0EnvValue(api, "MEM0_AGENT_ID");
  const orgId = getMem0EnvValue(api, "MEM0_ORG_ID");
  const projectId = getMem0EnvValue(api, "MEM0_PROJECT_ID");
  const customInstructions = getMem0EnvValue(api, "MEM0_CUSTOM_INSTRUCTIONS");
  const customPrompt = getMem0EnvValue(api, "MEM0_CUSTOM_PROMPT");
  const autoCapture = parseOptionalBoolean(getMem0EnvValue(api, "MEM0_AUTO_CAPTURE"));
  const autoRecall = parseOptionalBoolean(getMem0EnvValue(api, "MEM0_AUTO_RECALL"));
  const enableGraph = parseOptionalBoolean(getMem0EnvValue(api, "MEM0_ENABLE_GRAPH"));
  const searchThreshold = parseOptionalNumber(getMem0EnvValue(api, "MEM0_SEARCH_THRESHOLD"));
  const topK = parseOptionalNumber(getMem0EnvValue(api, "MEM0_TOP_K"));
  const customCategories = parseOptionalJsonObject(
    getMem0EnvValue(api, "MEM0_CUSTOM_CATEGORIES_JSON"),
  );
  const oss = parseOptionalJsonObject(getMem0EnvValue(api, "MEM0_OSS_JSON"));
  ...
  return Object.values(envConfig).some((value) => value !== undefined)
    ? envConfig
    : undefined;
}

function resolveMem0RawConfig(api: OpenClawPluginApi): unknown {
  if (hasConfigObject(api.pluginConfig) && Object.keys(api.pluginConfig).length > 0) {
    return api.pluginConfig;
  }
  return resolveMem0EnvConfig(api);
}
...
const memoryPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    const cfg = mem0ConfigSchema.parse(resolveMem0RawConfig(api));
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          ...
          try {
            ...
            api.logger.info(
              `openclaw-mem0-plugin: memory_store: userId=${uid}, runId=${runId}, content=${text}`,
            );
